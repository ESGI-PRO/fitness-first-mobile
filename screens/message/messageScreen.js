...
const MessageScreen = ({ navigation, route }) => {
  ...
  async function getMessages() {
    ...
    await messengerAPI.getMessages(item?.id).then((responses) => {
      const m = responses.map((message) => {
        return {
          id: message._id,
          message: message?.message,
          sender_id: message?.sender_id?.id,
          time: message?.createdAt,
          isSender: message?.sender_id?.id === user.userId ? true : false,
        };
      });
        ...
        >
          <View
            style={{
              ...
              backgroundColor:
                item.isSender == true ? Colors.primaryColor : "#D0D0D0",
            }}
          >
            <Text
              style={{
